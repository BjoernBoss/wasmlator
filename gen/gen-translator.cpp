...
gen::Translator::Translator(wasm::Module& mod, gen::TranslationInterface* interface, size_t maxDepth) : pAddresses{ mod, maxDepth }, pInterface{ interface } {
...
void gen::Translator::fProcess(const detail::OpenAddress& next) {
	...
	do {
		...
		while (true) {
			gen::Instruction inst = pInterface->fetch(address);
