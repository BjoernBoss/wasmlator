...
sys::detail::impl::ProcDirectory::ProcDirectory(detail::Syscall* syscall) : VirtualFileNode{ u8"/proc", fs::RootOwner, fs::RootGroup, fs::ReadExecute }, pSyscall{ syscall } {}
std::shared_ptr<sys::detail::FileNode> sys::detail::impl::ProcDirectory::spawn(const std::u8string& path, std::u8string_view name) {
	if (name == u8"self")
		return std::make_shared<impl::LinkFileNode>(path, str::u8::Int(pSyscall->config().pid), fs::RootOwner, fs::RootGroup, fs::ReadOnly);
...
sys::detail::impl::ProcId::ProcId(detail::Syscall* syscall, uint32_t id) : VirtualFileNode{ str::u8::Build(u8"/proc/", id), fs::RootOwner, fs::RootGroup, fs::ReadExecute }, pSyscall{ syscall } {}
std::shared_ptr<sys::detail::FileNode> sys::detail::impl::ProcId::spawn(const std::u8string& path, std::u8string_view name) {
	if (name == u8"exe" && pId == pSyscall->config().pid)
		return std::make_shared<impl::LinkFileNode>(path, pSyscall->config().path, fs::RootOwner, fs::RootGroup, fs::ReadOnly);
	return std::make_shared<impl::NullFileNode>(path);
}
