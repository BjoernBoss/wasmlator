...
env::Memory::Memory(env::Environment& environment, std::u8string moduleName, uint32_t cacheSize) : pEnvironment{ &environment }, pRootModuleName{ moduleName }, pCacheCount{ cacheSize } {
...
void env::Memory::fMakeAddress(wasm::Sink& sink, const env::MemoryState& state, uint32_t cache, const wasm::Variable& i64Address, const wasm::Function& lookup, env::MemoryType type) const {
	if (cache >= pCacheCount)
		pEnvironment->fail(u8"Cache [", cache, u8"] out of bounds as only [", pCacheCount, u8"] caches have been defined");
...
void env::Memory::fMakeWrite(wasm::Sink& sink, const wasm::Variable& i64Address, const env::MemoryState& state, uint32_t cache, env::MemoryType type) const {
...
void env::Memory::fMakeAccess(wasm::Module& mod, const env::MemoryState& state, const wasm::Prototype& readPrototype, const wasm::Prototype& writePrototype, std::u8string_view name, env::MemoryType type) const {
	...
	/* make the read and execute function */
	fMakeRead(read, read.parameter(0), state, pReadCache, type);
	fMakeExecute(execute, execute.parameter(0), state, pExecuteCache, type);

	/* make the write function (requires the value on the stack first) */
	write[I::Local::Get(write.parameter(1))];
	fMakeWrite(write, write.parameter(0), state, pExecuteCache, type);
}
...
uint32_t env::Memory::fReadi32Fromi8(env::addr_t address) const {
	return bridge::Memory::Readi32Fromi8(pEnvironment->identifier(), address);
}
uint32_t env::Memory::fReadi32Fromu8(env::addr_t address) const {
	return bridge::Memory::Readi32Fromu8(pEnvironment->identifier(), address);
}
uint32_t env::Memory::fReadi32Fromi16(env::addr_t address) const {
	return bridge::Memory::Readi32Fromi16(pEnvironment->identifier(), address);
}
uint32_t env::Memory::fReadi32Fromu16(env::addr_t address) const {
	return bridge::Memory::Readi32Fromu16(pEnvironment->identifier(), address);
}
uint32_t env::Memory::fReadi32(env::addr_t address) const {
	return bridge::Memory::Readi32(pEnvironment->identifier(), address);
}
uint64_t env::Memory::fReadi64(env::addr_t address) const {
	return bridge::Memory::Readi64(pEnvironment->identifier(), address);
}
float env::Memory::fReadf32(env::addr_t address) const {
	return bridge::Memory::Readf32(pEnvironment->identifier(), address);
}
double env::Memory::fReadf64(env::addr_t address) const {
	return bridge::Memory::Readf64(pEnvironment->identifier(), address);
}
void env::Memory::fWritei32Fromi8(env::addr_t address, uint32_t value) const {
	bridge::Memory::Writei32Fromi8(pEnvironment->identifier(), address, value);
}
void env::Memory::fWritei32Fromu8(env::addr_t address, uint32_t value) const {
	bridge::Memory::Writei32(pEnvironment->identifier(), address, value);
}
void env::Memory::fWritei32Fromi16(env::addr_t address, uint32_t value) const {
	bridge::Memory::Writei32Fromi16(pEnvironment->identifier(), address, value);
}
void env::Memory::fWritei32Fromu16(env::addr_t address, uint32_t value) const {
	bridge::Memory::Writei32Fromu16(pEnvironment->identifier(), address, value);
}
...
void env::Memory::fWritei64(env::addr_t address, uint64_t value) const {
	bridge::Memory::Writei64(pEnvironment->identifier(), address, value);
}
void env::Memory::fWritef32(env::addr_t address, float value) const {
	bridge::Memory::Writef32(pEnvironment->identifier(), address, value);
}
void env::Memory::fWritef64(env::addr_t address, double value) const {
	bridge::Memory::Writef64(pEnvironment->identifier(), address, value);
}
uint32_t env::Memory::fExecutei32Fromi8(env::addr_t address) const {
	return bridge::Memory::Executei32Fromi8(pEnvironment->identifier(), address);
}
uint32_t env::Memory::fExecutei32Fromu8(env::addr_t address) const {
	return bridge::Memory::Executei32Fromu8(pEnvironment->identifier(), address);
}
uint32_t env::Memory::fExecutei32Fromi16(env::addr_t address) const {
	return bridge::Memory::Executei32Fromi16(pEnvironment->identifier(), address);
}
uint32_t env::Memory::fExecutei32Fromu16(env::addr_t address) const {
	return bridge::Memory::Executei32Fromu16(pEnvironment->identifier(), address);
}
uint32_t env::Memory::fExecutei32(env::addr_t address) const {
	return bridge::Memory::Executei32(pEnvironment->identifier(), address);
}
uint64_t env::Memory::fExecutei64(env::addr_t address) const {
	return bridge::Memory::Executei64(pEnvironment->identifier(), address);
}
float env::Memory::fExecutef32(env::addr_t address) const {
	return bridge::Memory::Executef32(pEnvironment->identifier(), address);
}
double env::Memory::fExecutef64(env::addr_t address) const {
	return bridge::Memory::Executef64(pEnvironment->identifier(), address);
}
...
env::MemoryState env::Memory::setupImports(wasm::Module& mod) const {
	...
	state.memory = mod.memory(wasm::Limit{ env::MinPages }, u8"mem_core", wasm::Import{ pRootModuleName, u8"mem_core" });
	state.caches = mod.memory(wasm::Limit{ pCachePages, pCachePages }, u8"mem_cache", wasm::Import{ pRootModuleName, u8"mem_cache" });

	/* add the function-imports for the page-lookup */
	wasm::Prototype prototype = mod.prototype({ { u8"addr", wasm::Type::i64 }, { u8"size", wasm::Type::i32 }, { u8"cache", wasm::Type::i32 } }, { wasm::Type::i32 }, u8"mem_addr_lookup");
	state.readFunction = mod.function(prototype, u8"mem_lookup_read", wasm::Import{ pRootModuleName, u8"mem_lookup_read" });
	state.writeFunction = mod.function(prototype, u8"mem_lookup_write", wasm::Import{ pRootModuleName, u8"mem_lookup_write" });
	state.executeFunction = mod.function(prototype, u8"mem_lookup_execute", wasm::Import{ pRootModuleName, u8"mem_lookup_execute" });
	return state;
}
...
void env::Memory::makeWrite(const wasm::Variable& i64Address, const env::MemoryState& state, uint32_t cache, env::MemoryType type) const {
	wasm::Sink& sink = wasm::Variable{ i64Address }.sink();
	return fMakeWrite(sink, i64Address, state, cache, type);
}
