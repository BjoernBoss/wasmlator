...
size_t trans::detail::SuperBlock::fLookup(env::guest_t address) const {
...
void trans::detail::SuperBlock::fSetupRanges() {
...
bool trans::detail::SuperBlock::fExpandRanges() {
...
void trans::detail::SuperBlock::fFixRanges() {
	host::Fail(u8"Irreducible control-flow within a super-block cannot yet be translated!");
}

bool trans::detail::SuperBlock::push(const trans::Instruction& inst) {
	pList.push_back(inst);

	/* check if the current strand can be continued */
	return (inst.type != trans::InstType::endOfBlock && inst.type != trans::InstType::jumpDirect);
}
bool trans::detail::SuperBlock::incomplete(env::guest_t address) const {
	...
	*	Note: Ignore the case of a jump into an instruction, as it needs to be translated as a new super-block anyways */
	for (size_t i = 0; i < pList.size(); ++i) {
		if (pList[i].type != trans::InstType::jumpDirect && pList[i].type != trans::InstType::conditionalDirect)
			continue;
...
void trans::detail::SuperBlock::setupRanges() {
...
const wasm::Target* trans::detail::SuperBlock::lookup(env::guest_t address) const {
...
trans::detail::InstChunk trans::detail::SuperBlock::next(wasm::Sink& sink) {
