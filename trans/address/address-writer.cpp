...
trans::detail::AddressWriter::AddressWriter(const detail::MappingState& mapping, detail::Addresses& host, wasm::Sink& sink) : pMapping{ mapping, sink }, pHost{ host }, pSink{ sink } {}

void trans::detail::AddressWriter::fCallLandingPad(env::guest_t nextAddress) const {
	if (!pTempRetState.valid())
		pTempRetState = pSink.local(wasm::Type::i32, u8"addr_temp_state");
	if (!pTempRetAddress.valid())
		pTempRetAddress = pSink.local(wasm::Type::i64, u8"addr_temp_address");
...
void trans::detail::AddressWriter::makeCall(env::guest_t address, env::guest_t nextAddress) const {
	...
	/* add the lookup via the table */
		...
		if (!target.alreadyExists) {
			...
			if (!pTempFuncIndex.valid())
				pTempFuncIndex = pSink.local(wasm::Type::i32, u8"addr_temp_function");
...
void trans::detail::AddressWriter::makeCallIndirect(env::guest_t nextAddress) const {
	/* ensure that the local variables exist */
	if (!pTempFuncIndex.valid())
		pTempFuncIndex = pSink.local(wasm::Type::i32, u8"addr_temp_function");
...
void trans::detail::AddressWriter::makeJump(env::guest_t address) const {
	detail::PlaceAddress target = pHost.pushLocal(address);
...
void trans::detail::AddressWriter::makeJumpIndirect() const {
	/* ensure that the local variables exist */
	if (!pTempFuncIndex.valid())
		pTempFuncIndex = pSink.local(wasm::Type::i32, u8"addr_temp_function");
...
void trans::detail::AddressWriter::makeReturn() const {
